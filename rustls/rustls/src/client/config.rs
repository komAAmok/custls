...
pub struct ClientConfig {
    ...
    /// Optional ClientHello customizer for fingerprint simulation.
    /// The default is None (no customization).
    pub custls_customizer: Option<Arc<dyn crate::custls::ClientHelloCustomizer>>,
}
...
impl ClientConfig {
    ...
    /// Enable custls ClientHello customization with the provided customizer.
    ///
    /// This allows sophisticated fingerprint simulation and ClientHello modification.
    /// See the `custls` module documentation for details.
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    /// config.enable_custls(customizer);
    /// ```
    pub fn enable_custls(&mut self, customizer: Arc<dyn crate::custls::ClientHelloCustomizer>) {
        self.custls_customizer = Some(customizer);
    }

    /// Disable custls ClientHello customization.
    ///
    /// This provides zero-overhead mode where no customization hooks are invoked.
    /// This is the default state for a new `ClientConfig`.
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    /// #     .with_root_certificates(rustls::RootCertStore::empty())
    /// #     .with_no_client_auth();
    /// config.disable_custls();
    /// ```
    pub fn disable_custls(&mut self) {
        self.custls_customizer = None;
    }

    /// Check if custls customization is enabled.
    ///
    /// Returns `true` if a customizer is configured, `false` otherwise.
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    /// #     .with_root_certificates(rustls::RootCertStore::empty())
    /// #     .with_no_client_auth();
    /// assert!(!config.is_custls_enabled());
    /// ```
    pub fn is_custls_enabled(&self) -> bool {
        self.custls_customizer.is_some()
    }
}
